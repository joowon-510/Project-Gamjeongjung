...
export const useChat = ({ 
  ...
  // 메시지 ID 생성 함수
  const generateMessageId = useCallback((msg: SendWebSocketMessage | Message): string => {
    if ('id' in msg) return msg.id;
    return `msg_${msg.createdAt}_${msg.sender}_${Math.random().toString(36).substring(2, 9)}`;
  }, []);

  // 메시지 처리
  const processIncomingMessage = useCallback((websocketMessage: WebSocketMessage) => {
    if (websocketMessage.type === MessageType.MESSAGE) {
      ...
      if (Number(receiveMsg.receiver) !== Number(userId)) {
        ...
        setMessages(prev => prev.map(msg => {
          if (msg.isMe && !msg.read && new Date(msg.timestamp).getTime() <= receiveTime) {
  ...
  }, [userId, roomId, recipientName, generateMessageId, chatService, updateLastReceivedMessage]);
