    ...
    try {
      ...
      if (response.data && response.data.status_code === 200) {
        ...
        if (response.data.body) {
          ...
          const chatItemMap = chatItemMapString
            ? JSON.parse(chatItemMapString)
            : {};
          chatItemMap[roomId] = itemId;
          ...
          const chatRoomId =
            response.data.body?.roomId || `${sellerId}_${itemId}_${Date.now()}`;

          // 🔽 itemId 저장 추가
          const chatItemMapString = localStorage.getItem(CHAT_ITEM_KEY);
          const chatItemMap = chatItemMapString
            ? JSON.parse(chatItemMapString)
            : {};
          chatItemMap[chatRoomId] = itemId;
          localStorage.setItem(CHAT_ITEM_KEY, JSON.stringify(chatItemMap));
